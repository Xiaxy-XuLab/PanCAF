...
marker <- read.table("new_marker.txt", header = T, sep = "\t", row.names = 1)

dir <- list.files("TCGA")
setwd("TCGA")
data_set_c6 <- list(enriched = marker[
    which(marker$cluster == 6 & marker$p_val_adj < 0.05), "gene"
][1:50])
data_set_c19 <- list(enriched = marker[
    which(marker$cluster == 19 & marker$p_val_adj < 0.05), "gene"
][1:50])

plot_list <- foreach::foreach(gp = 1:length(dir)) %do% {
    bulk <- read.table(dir[k], header = T, sep = "\t")
    bulk <- bulk[-which(duplicated(bulk$Hybridization.REF)), ]
    rownames(bulk) <- bulk$Hybridization.REF
    bulk <- bulk[, -1]
    result6 <- gsva(as.matrix(bulk), data_set_c6,
        min.sz = 5,
        kcdf = "Poisson", method = "ssgsea",
        mx.diff = TRUE, verbose = FALSE, parallel.sz = 30
    )
    result19 <- gsva(as.matrix(bulk), data_set_c19,
        min.sz = 5,
        kcdf = "Poisson", method = "ssgsea",
        mx.diff = TRUE, verbose = FALSE, parallel.sz = 30
    )
    ...
    mat <- data.frame(
        c6 = as.vector(result6),
        c19 = as.vector(result19)
    )
    ggplot(mat, aes(x = c6, y = c19)) +
        geom_point(size = 3, color = "#B1CF94") +
        geom_smooth(method = "lm", se = FALSE, color = "black") +
        stat_cor(method = "pearson", label.x = max(mat$c6) * 0.2) +
        theme_classic() +
        labs(title = sapply(strsplit(dir[k], ".", fixed = T), "[", 1)) +
        theme(plot.title = element_text(hjust = 0.5, color = "black"))
}
final <- wrap_plots(plot_list, ncol = 8, guides = "collect")
ggsave("TCGA_COR_RESULT.pdf", final, width = 24, height = 15)
